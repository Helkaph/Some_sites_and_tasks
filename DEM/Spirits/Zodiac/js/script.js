...
Spirit_Identidy.addEventListener('click', () => {
    ...
    <p>Рост: 148см; замеры тела - B91/W60/H88</p>
    ...
    <p>При использовании способности <<Шифуру>> Астральное Одеяние немного изменяется: платье становится более плотным и откровенным, появляются фиолетовые чулки, перчатки становятся длиннее. Волосы заплетаются в два хвоста.</p>`
...
Spirit_Persona.addEventListener('click', () => {
    ...
    <p>После событий с Да'атом показывает истинную личность. Обладает маниакальной привязанностью к любимым. Эмоции чисты и невинны. Говорит о себе в третьем лице устаревшим слогом.</p>`
})
Spirit_Power.addEventListener('click', () => {
    ...
    <br>
    <p class="postscriptum"><i>Примечание:</i> в скобках указаны истинные параметры, доступные после использования соответствующей способности <<Шифуру>></p>`
})